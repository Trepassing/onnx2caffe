...
def _convert_Add(node, graph, err):
    ...
    # print(graph.channel_dims.keys())
    graph.channel_dims['528'] = 0  # for yolact
    graph.channel_dims[output_name] = graph.channel_dims[input_name_list[0]]
    return layer
...
def _convert_pool(node, graph, err):
    ...
    pads = node.attrs.get('pads', [0, 0, 0, 0])
    print(node_name)
    print(pads, strides, kernel_shape)
    if node_name == 'MaxPool_2':
        pads = [0, 0, 0, 0]
...
def _convert_resize_seg(node,graph,err):
    # factor_list = node.input_tensors.get(node.inputs[1])
    ...
    #https://github.com/pytorch/pytorch/issues/6900
    if  str(mode) == "linear": #mode=="linear":
        channels = graph.channel_dims[input_name]
        outputshape = node.input_tensors.get(node.inputs[3])
        # inputshape = node.input_tensors.get(node.inputs[0])
        inputshape = graph.shape_dict[input_name]
        print(outputshape, inputshape)
        
        if outputshape[-1] / inputshape[-1] == 2:
            scales = [1,1,2,2]
        elif outputshape[-1] / inputshape[-1] == 4:
            scales = [1,1,4,4]
        else:
            if node_name == 'Resize_130':
                scales = [1,1,2,2]
            else:
                scales = [1,1,8,8]
        height_scale = int(scales[2])
...
def _convert_resize_(node, graph, err):
    ...
    if node.name == 'Resize_118':
        upsample_h = 40
        upsample_w = 40
    if node.name == 'Resize_140':
        upsample_h = 80
        upsample_w = 80
    layer = myf("Upsample", node_name, [input_name], [output_name],
                upsample_param=dict(
                    upsample_h = int(upsample_h),
                    upsample_w = int(upsample_w)
                ))
    graph.channel_dims[output_name] = graph.channel_dims[input_name]
    return layer
...
def _convert_conv_transpose(node, graph, err):  # 反卷积
    ...
    pads = node.attrs.get("pads", [0, 0, 0, 0])
    strides = node.attrs["strides"]
    # if node_name == "ConvTranspose_166":
    #     outputnumber = 
    layer = myf('Deconvolution', node_name, [input_name], [output_name],
                convolution_param=dict(
